        ...
        def profile_step():
            ...
            if last_start_time is not None and self.dist.rank == 0:
                try:
                    ...
                    if iter_states:
                        num_ctx_requests = iter_states.get(
                            'num_ctx_requests', 0)
                        num_ctx_tokens = iter_states.get('num_ctx_tokens', 0)
                        num_generation_tokens = iter_states.get(
                            'num_generation_tokens', 0)

                        total_running = num_ctx_requests + num_generation_tokens / (
                            1 + self.model_engine.max_draft_len)
                        prom_metrics["num_requests_running"] = total_running
                        prom_metrics[
                            "num_requests_swapped"] = total_running - len(
                                self.active_requests)

                        prom_metrics[
                            "iteration_tokens_total_sum"] += num_ctx_tokens + num_generation_tokens
                        prom_metrics["iteration_tokens_total_count"] += 1
                        prom_metrics["time_per_output_token_seconds_sum"] += (
                            start_time - last_start_time)
                        prom_metrics["time_per_output_token_seconds_count"] += 1
                        prom_metrics["prompt_tokens_total"] += num_ctx_tokens
                        prom_metrics[
                            "request_prompt_tokens_total_sum"] += num_ctx_tokens
                        prom_metrics["request_prompt_tokens_total_count"] += 1
                        prom_metrics[
                            "generation_tokens_total"] += num_generation_tokens
                        prom_metrics[
                            "request_generation_tokens_total_sum"] += num_generation_tokens
                        prom_metrics[
                            "request_generation_tokens_total_count"] += 1

                        write_metrics_to_file()
                except Exception:
                    # Silently fail if metrics operations fail
                    pass
