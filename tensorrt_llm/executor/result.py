...
class GenerationResultBase:
    ...
    def record_stats(self,
        ...
        if self.prompt_token_ids:
            metrics_stats[MetricNames.PROMPT_TOKENS] = len(self.prompt_token_ids)
        if self.cached_tokens > 0:
            metrics_stats[MetricNames.PROMPT_CACHE_CACHED_TOKENS] = \
                self.cached_tokens
        # Speculative decoding: prefer per-position arrays from PyTorch,
        # fall back to C++ aggregate totals.
        ...
        # Perplexity: exp(-mean(logprobs)) over the relevant tokens.
        # Prefill perplexity from prompt_logprobs (per-prompt-token logprobs).
        if output.prompt_logprobs and self.prompt_token_ids:
            try:
                prompt_lps = []
                for i, entry in enumerate(output.prompt_logprobs):
                    if i >= len(self.prompt_token_ids):
                        break
                    token_id = self.prompt_token_ids[i]
                ...
                logger.debug("Failed to compute prefill perplexity", exc_info=True)

        # Generation perplexity from cumulative_logprob / num_generated_tokens,
        # or from per-token generation logprobs.
            ...
            if output.cumulative_logprob is not None:
                ...
                try:
                    ...
                    for i, entry in enumerate(output.logprobs):
                        if isinstance(entry, dict):
                            token_id = output.token_ids[i] if i < len(output.token_ids) else None
                    ...
                    if gen_lps:
                        gen_ppl = math.exp(-sum(gen_lps) / len(gen_lps))
                except Exception:
                    logger.debug("Failed to compute generation perplexity from logprobs", exc_info=True)
